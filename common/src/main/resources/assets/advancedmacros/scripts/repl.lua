...
function R.setup()
	...
	REPL.setOnKeyPressed(function(...)
		if REPL.elements.input.isFocused() then
			return
		end
		REPL.elements.input.setFocused(true)
		local char = ""
		if __GAME_VERSION == "1.12.2" then
			...
			char = charTyped
		elseif __GAME_VERSION == "1.14.4" or __GAME_VERSION == "1.15.2" or __GAME_VERSION == "1.20.4" then
			local keyName, scanCode, modifiers = ...
			char = keyName
		else
			toast("REPL", "Unsupported version")
		end
		if string.byte(char) < string.byte(" ") then
			return
		end --ignore control chars
		REPL.elements.input.setText(REPL.elements.input.getText() .. char)
	end)
	if __GAME_VERSION == "1.12.2" then
		REPL.elements.input.setOnKeyPressed(function(char, keyCode)
			--log(keyCode)
	...
	elseif __GAME_VERSION == "1.14.4" or __GAME_VERSION == "1.15.2" or __GAME_VERSION == "1.20.4" then
		REPL.elements.input.setOnKeyPressed(function(keyName, keyCode, mods)
			if keyName == "ENTER" then
				...
				REPL.evaluate(expr)
				REPL.historyN = 0
			elseif keyName == "UP" then --up
				REPL.historyN = math.min((REPL.historyN or 0) + 1, #REPL.history)
				if REPL.historyN > 0 then
					REPL.elements.input.setText(REPL.history[REPL.historyN])
				end
			elseif keyName == "DOWN" then --down
				REPL.historyN = math.max(0, (REPL.historyN or 0) - 1)
				if REPL.historyN > 0 then
					REPL.elements.input.setText(REPL.history[REPL.historyN])
				else
	...
	function REPL.updateVarList()
		...
		for k, v in lexPairs(t) do
			local typeColor = ({ tab = "&a", num = "&b", boo = "&e", fun = "&d", str = "&6" })[type(v):sub(1, 3)]
				or ("&7[" .. type(v) .. "]&f")
			local first = true
			local vs = tostring(v)
			for w in vs:gmatch("[^\n]+") do
		...
		for i = 1, 500 do
			if lines[i] then
				...
				if not REPL.elements.varLines[i] then
					REPL.elements.varLines[i] = REPL.newText(
						"",
						wid * 2 / 3 + 5,
					...
					REPL.elements.varLines[i].setOnMouseClick(function(x, y, button)
						if button == 0 then --LMB
							...
							REPL.elements.input.setText(
								REPL.elements.input.getText()
									.. msg:sub(2)
									.. (type(REPL.elements.varLines[i].varValue) == "function" and "(" or "")
							)
						end
					end)
				end
		...
		REPL.elements.varScroll.setScrollPos(
			math.min(REPL.elements.varScroll.getScrollPos(), REPL.elements.varScroll.getMaxItems())
		)
		REPL.elements.varHScroll.setScrollPos(
			math.min(REPL.elements.varHScroll.getScrollPos(), REPL.elements.varHScroll.getMaxItems())
		)
	...
	function REPL.updateVarScrollOffset()
		...
		REPL.elements.varGroup.setPos(
			wid * 2 / 3 + 5 - REPL.elements.varHScroll.getScrollPos(),
			-REPL.elements.varScroll.getScrollPos() * 12
		)
	end
	...
	function REPL.evaluate(expr)
		...
		if
			REPL.activeEval
			and (
				REPL.activeEval.getStatus()
				and (
					REPL.activeEval.getStatus() == "STOPPED" --pcall should stop this from being needed
					or REPL.activeEval.getStatus() == "CRASH"
					or REPL.activeEval.getStatus() == "DONE"
				)
			)
		then
			REPL.activeEval = nil
		...
		REPL.echo(
			(REPL.elements.modeIcon.mode == "global" and "&7[&1G&7] &f" or "&7[&aS&7]&f ")
				.. expr:gsub("([^&])&([abcdef1234567890BIOSUFNTR])", "%1&&%2")
		)
