...
public class TaskDispatcher {
    ...
    private static class DelayedTask<Z> {
        ...
        public DelayedTask(Callable<Z> t, long exTime) {
            task = t;
            executionTime = exTime + System.currentTimeMillis();
        }

        public DelayedTask(Runnable t, long exTime) {
            task = t;
            executionTime = exTime + System.currentTimeMillis();
        }
        ...
        public void addTask() {
            if (task instanceof Runnable) {
                TaskDispatcher.addTask((Runnable) task);
            } else {
                TaskDispatcher.addTask((Callable<Z>) task);
            }
        }
    ...
    private static <Z> void insertSorted(ArrayList<DelayedTask<?>> list, DelayedTask<Z> task) {
        synchronized (delayedTasks) {
            int cursor = 0;
            while (cursor < list.size() && list.get(cursor).executionTime > task.executionTime) {
                cursor++;
            }
