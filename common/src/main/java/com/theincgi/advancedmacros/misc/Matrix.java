...
public class Matrix {
    ...
    public Matrix mult(Matrix b) {
        if (this.COLS != b.ROWS) {
            throw new RuntimeException(String.format("Can't multipy matrix sizes <R: %d, C: %d> * <R: %d, C: %d>", this.ROWS, this.COLS, b.ROWS, b.COLS));
        }
    ...
    public Matrix add(Matrix b) {
        if (this.COLS != b.COLS && this.ROWS != b.ROWS) {
            throw new RuntimeException(String.format("Can't add matrix sizes <R: %d, C: %d> + <R: %d, C: %d>", this.ROWS, this.COLS, b.ROWS, b.COLS));
        }
        Matrix out = new Matrix(this.ROWS, b.COLS);
        for (int r = 0; r < ROWS; r++) {
    ...
    public Matrix rotate(Axis axis, float angle) {
        if (this.COLS == 1 && this.ROWS == 3) {
            ...
            if (axis.equals(Axis.X)) {
                ...
                throw new RuntimeException("Unknown axis enum");
            }
