...
public class CustomFontRenderer {
    ...
    private void drawChar3D(Matrix4f transform, float x, float y, float z, Matrix upVect, Matrix rightVect, char c) {
        ...
        BufferBuilder buffer = Tessellator.getInstance().getBuffer();
        buffer.begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_TEXTURE);
        buffer.vertex(transform, x, y, z).texture(uvPair.umax, uvPair.vmax).next(); //bottom left
        buffer.vertex(transform, x + ux, y + uy, z + uz).texture(uvPair.umax, uvPair.vmin).next(); //top left
        buffer.vertex(transform, x + ux + rx, y + uy + ry, z + uz + rz).texture(uvPair.umin, uvPair.vmin).next(); //top right
        buffer.vertex(transform, x + rx, y + ry, z + rz).texture(uvPair.umin, uvPair.vmax).next(); //bottom right
        Tessellator.getInstance().draw();
    }
