...
public class CallableTable extends LuaTable {
    ...
    public LuaValue getJsonLuaDoc() {
        JsonElement jObj = getObjectFromJson(docName);
        if (jObj != null && jObj.isJsonObject() && jObj.getAsJsonObject().has("luaDoc")) {
    ...
    public LuaValue getJsonArgTypes() {
        JsonElement jObj = getObjectFromJson(docName);
        if (jObj != null && jObj.isJsonObject() && jObj.getAsJsonObject().has("types")) {
    ...
    /**
     * defaults to en_us if it can't load any other files, null otherwise
     */
