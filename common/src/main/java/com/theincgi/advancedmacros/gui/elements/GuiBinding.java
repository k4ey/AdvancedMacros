...
public class GuiBinding implements Moveable, Drawable, InputSubscriber, IBinding {
    ...
    private File scriptHome(String script2) {
        if (script2 == null) {
            return AdvancedMacros.MACROS_FOLDER;
        }
        if (script2.contains("/")) {
            return new File(AdvancedMacros.MACROS_FOLDER, script.substring(0, script2.lastIndexOf('/')));
        }
        return AdvancedMacros.MACROS_FOLDER;
    }
    ...
    public boolean onScroll(Gui gui, double i) {
        ...
        if (eventSelector.onScroll(gui, i)) {
            return true;
        }
        return false;
    }
    ...
    public boolean onMouseClick(Gui gui, double x, double y, int buttonNum) {
        ...
        if (eventSelector.isOpen() && !eventSelector.isInBounds(x, y)) {
            eventSelector.close();
            return true;
        }
        //if(scriptSelector.isOpen() && !scriptSelector.isInBounds(x, y)){scriptSelector.close(); return true;}
        if (pickScript.onMouseClick(gui, x, y, buttonNum)) {
            return true;
        }
        if (removeButton.onMouseClick(gui, x, y, buttonNum)) {
            return true;
        }
        if (enableButton.onMouseClick(gui, x, y, buttonNum)) {
            return true;
        }
        if (modeButton.onMouseClick(gui, x, y, buttonNum)) {
            return true;
        }
        if (eventSelector.onMouseClick(gui, x, y, buttonNum)) {
            return true;
        }
        //if(scriptSelector	.onMouseClick(gui, x, y, buttonNum))return true;
        if (editButton.onMouseClick(gui, x, y, buttonNum)) {
            return true;
        }
        if (moveButton.onMouseClick(gui, x, y, buttonNum)) {
            return true;
        }
        return false;
    }
    ...
    public boolean onMouseRelease(Gui gui, double x, double y, int state) {
        if (removeButton.onMouseRelease(gui, x, y, state)) {
            return true;
        }
        if (enableButton.onMouseRelease(gui, x, y, state)) {
            return true;
        }
        if (modeButton.onMouseRelease(gui, x, y, state)) {
            return true;
        }
        if (eventSelector.onMouseRelease(gui, x, y, state)) {
            return true;
        }
        //if(scriptSelector	.onMouseRelease(gui, x, y, state))return true;
        if (pickScript.onMouseRelease(gui, x, y, state)) {
            return true;
        }
        if (editButton.onMouseRelease(gui, x, y, state)) {
            return true;
        }
        if (moveButton.onMouseRelease(gui, x, y, state)) {
            return true;
        }
        return false;
    }
    ...
    public boolean onMouseClickMove(Gui gui, double x, double y, int buttonNum, double q, double r) {
        if (removeButton.onMouseClickMove(gui, x, y, buttonNum, q, r)) {
            return true;
        }
        if (enableButton.onMouseClickMove(gui, x, y, buttonNum, q, r)) {
            return true;
        }
        if (modeButton.onMouseClickMove(gui, x, y, buttonNum, q, r)) {
            return true;
        }
        if (eventSelector.onMouseClickMove(gui, x, y, buttonNum, q, r)) {
            return true;
        }
        if (pickScript.onMouseClickMove(gui, x, y, buttonNum, q, r)) {
            return true;
        }
        if (editButton.onMouseClickMove(gui, x, y, buttonNum, q, r)) {
            return true;
        }
        if (moveButton.onMouseClickMove(gui, x, y, buttonNum, q, r)) {
            return true;
        }
        return false;
    }
