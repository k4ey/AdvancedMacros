...
public class ColorTextArea implements Drawable, InputSubscriber, Moveable, Focusable {
    ...
    public ColorTextArea(PropertyPalette propPalette, Gui g) {
        ...
        propPalette
                .addColorIfNil(new Color(255, 128, 128, 128), "colors", "textFill")
                .addColorIfNil(new Color(255, 255, 255, 255), "colors", "frame")
                .addColorIfNil(new Color(255, 0, 0, 0), "colors", "plainText")
                .addColorIfNil(new Color(255, 0, 80, 100), "colors", "keyword")
                .addColorIfNil(new Color(255, 0, 85, 0), "colors", "comment")
                .addColorIfNil(new Color(255, 0, 109, 9), "colors", "variable")
                .addColorIfNil(new Color(60, 255, 128, 64), "colors", "stringBoxFill")
                .addColorIfNil(new Color(255, 255, 128, 64), "colors", "stringBoxFrame")
                .addColorIfNil(new Color(255, 128, 255, 255), "colors", "function")
                .addColorIfNil(new Color(255, 120, 250, 100), "colors", "table")
                .addColorIfNil(new Color(255, 91, 182, 255), "colors", "cursorOn")
                .addColorIfNil(new Color(255, 0, 132, 79), "colors", "selectionFrame")
                .addColorIfNil(new Color(255, 57, 206, 146), "colors", "selectionFill");
        ...
        lines = new ArrayList<String>() {
            ...
            public String get(int index) {
                if (index < 0 || index >= size()) {
                    return "";
                } //no null pointer for when view is outside the text
    ...
    private void drawCursor(DrawContext drawContext, Gui g) {
        ...
        if (cursor.getX() - viewX < 0 || cursor.getX() - viewX > visChars[0].length || cursor.getY() - viewY < 0 || cursor.getY() - viewY > visChars.length) {
            return;
        }//offscreen
    ...
    private void solveTextColor() {
        ...
        for (int sLine = viewY, dY = 0; dY < visChars.length; sLine++, dY++) {
            ...
            while (quoteFind.find()) {
                ...
                for (int x = start; x < end; x++) {
                    ...
                    if ((line.charAt(end - 1) == ']' && x < end - 1) || (line.charAt(end - 1) != ']' && x < end)) {
                        if (x == start || (x == start + 1 && c == '[' && line.charAt(start) == '[')) {
                            continue;
                        }
                        try {
                            cols[dY][x - viewX].setFrom(textCode);
            ...
            while ((w = line.indexOf("--", w + 1)) != -1) {
                if (w > viewX + quote[0].length) {
                    break; /*off right side of screen, doesnt matter*/
                }
                for (Q q : quoteSpots) {
    ...
    public boolean onMouseClick(Gui gui, double x, double y, int buttonNum) {
        if (hBar.onMouseClick(gui, x, y, buttonNum)) {
            return textChanged = true;
        }
        if (vBar.onMouseClick(gui, x, y, buttonNum)) {
            return textChanged = true;
        }
    ...
    public boolean onMouseRelease(Gui gui, double x, double y, int state) {
        if (hBar.onMouseRelease(gui, x, y, state)) {
            return textChanged = true;
        }
        if (vBar.onMouseRelease(gui, x, y, state)) {
            return textChanged = true;
        }
        return false;
    }
    ...
    public boolean onMouseClickMove(Gui gui, double x, double y, int buttonNum, double q, double r) {
        if (hBar.onMouseClickMove(gui, x, y, buttonNum, q, r)) {
            return textChanged = true;
        }
        if (vBar.onMouseClickMove(gui, x, y, buttonNum, q, r)) {
            return textChanged = true;
        }
        return false;
    }
            ...
            default:
                if (isCTRLDown()) {
                    ...
                    if (keyCode == GLFW.GLFW_KEY_A) {
                        ...
                        try {
                            String data = GLFW.glfwGetClipboardString(MinecraftClient.getInstance().getWindow().getHandle());
    ...
    public File getScriptFile() {
        return new File(AdvancedMacros.MACROS_FOLDER, scriptName);
    }
    ...
    public void resize(int newWid, int newHei) {
        if (newWid == 0 || newHei == 0 || g.width == 0 || g.height == 0 || MinecraftClient.getInstance().getWindow().getWidth() == 0 || MinecraftClient.getInstance().getWindow().getHeight() == 0) {
            return;
        }
        if (wid != newWid || hei != newHei) { //something changed
            ...
            //			bufferedImage = new BufferedImage((int) (MinecraftClient.getInstance().displayWidth  * (newWid/(float)g.width)),
            //					(int) (MinecraftClient.getInstance().displayHeight * (newHei/(float)g.height)),
    ...
    /**
     * use AdvancedMacros.globals.getraw("_G");<br> for tmp, start with "" <br>
     */
    ...
    public void openScript(String scriptName) {
        ...
        lines.clear();
        File sScript = new File(AdvancedMacros.MACROS_FOLDER, scriptName);
        if (sScript.exists()) {
            try {
